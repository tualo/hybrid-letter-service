    ...
    class HttpServer extends Command {
      ...
      openExpressServer() {
        ...
        app.get('/hls/hybrid/list', (req, res) => {
          ...
          return prms = this.globJobFiles().then(function(data) {
            ...
            return prms_font = me.precheckfonts_loop(data).then(function(data) {
              console.log('/hls/hybrid/list');
      ...
      precheckfonts_loop(liste) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          running = Array(liste.length).fill(1);
          listFN = function(index) {
            var filename, item, prms;
            if (index < liste.length) {
              item = liste[index];
              filename = item.file.replace('.xml', '.pdf');
              prms = me.precheckfonts(filename).then(function(data) {
                ...
                if (columns[0] !== 'name') {
                  if (columns[0].indexOf('--') !== 0) {
                    ...
                    if (columns[3] === 'no' && columns[4] === 'no') {
                      result.success = false;
                      return result.message = 'Die Schriftart ' + columns[0] + ' kann nicht richtig verarbeitet werden';
                    }
                  }
                }
              }
      ...
      processCreatePDF(range) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          for (i = 0, len = range.length; i < len; i++) {
            ...
            if (record.color === 'Schwarz/Weiß' && record.envelope === 'DIN C6 (22,9cm x 11,4cm)') { //'DIN C6/5 (22,9cm x 11,4cm)'
