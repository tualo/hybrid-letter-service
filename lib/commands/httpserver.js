    ...
    class HttpServer extends Command {
      ...
      openExpressServer() {
        ...
        app.post('/hls/hybrid/print', (req, res) => {
          var cancelcups, files, me, result, running;
          me = this;
          result = {
            success: true
          };
      ...
      killoldjobs() {
        ...
        return lpstat = me.runcommand('lpstat', []).then(function(data, opt) {
          ...
          datalist = data.split("\n");
          return datalist.forEach(function(line) {
            var tabs;
            tabs = line.split(/\s/);
            return console.log(tabs[0]);
          });
        }).catch(function(data) {
          return console.log('killoldjobs*', data);
        });
      }
